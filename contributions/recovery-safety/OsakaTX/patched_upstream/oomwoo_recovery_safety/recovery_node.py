...
STALE_REPUBLISH_SEC = 0.25  # pause-note period; < monitor roster max_age (README example 0.5) (estimate; sim-tunable)
PAUSE_ANNOUNCE_WINDOW_SEC = 10.0  # bounded post-event announce window, then silence (estimate; sim-tunable)
...
class RecoverySafetyNode(Node):
    def __init__(self):
        ...
        self._last_republish_sec = float("-inf")
    ...
    def _health_stale_republish(self) -> None:
        """Bounded refreshing-republish of the last sampled beat (pause note).

        A paused robot emits no further work events, so the last heartbeat
        would otherwise go silent mid-pause. Conditions, all three required:
          1. period gate  - at most one republish per STALE_REPUBLISH_SEC
          2. event-age    - the sampled beat is stale (age >= period), so
                            live event beats are never shadowed
          3. finite window - announcing stops PAUSE_ANNOUNCE_WINDOW_SEC after
                            the last work event (long silences then read as
                            real liveness loss, not as a marker ticker)
        Marker detail "stale-repub" plus SAME sequence identify the republish;
        ``stamp_sec`` is re-measured from THIS node's clock at the real
        publication moment, so both sides of every age comparison come from
        ...
        if beat is None:
            return
        now = self._now_sec()
        if now - self._last_republish_sec < STALE_REPUBLISH_SEC:
            return
        if now - beat.stamp_sec < STALE_REPUBLISH_SEC:
            return
        if now - beat.stamp_sec > PAUSE_ANNOUNCE_WINDOW_SEC:
            self._last_republish_sec = now  # window closed; hold the gate
            return
        payload = beat.to_payload()
        payload["stamp_sec"] = now  # re-measured liveness beat (see docstring)
        payload["detail"] = "stale-repub" + (";" + beat.detail if beat.detail else "")
        self._health_pub.publish(String(data=json.dumps(payload, sort_keys=True)))
        self._last_republish_sec = now
