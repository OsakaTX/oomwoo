...
- The hardening latch (§4) requires an ack path design (operator or status-
  ...
  — **NOW DESIGNED (2026-08-23)**: `roe/pause_alert_ack.py` + `pause-alert-ack-path.md`
  specify clear-before-rearm ack admission, bounded re-annunciation/escalation,
  and a drift-guard on the deployed node's alert surface (3 publishers, one-shot
  status, True-only reset). The latch's `ack()` and the supervisor's
  `evaluate()` are two layers of the same never-rearm-against-a-live-level
  principle; wiring both into recovery_node.py remains a future node PR.
