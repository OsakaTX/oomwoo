...
| 2 | **Cliff** (`/oomwoo/safety/cliff`) | Immediate stop, publish SAFETY_CLIFF | Yes (bounded escape) | Auto escape / resume; pause-and-alert if exhausted |
| 3 | **Wheel drop** (`/oomwoo/safety/wheel_drop`) | Immediate stop, publish SAFETY_WHEEL_DROP | Yes (bounded escape) | Auto escape / resume; pause-and-alert if exhausted |
...
> **Ledge events (cliff / wheel-drop) — updated 2026-08-10.** The merged
> xbattlax controller pauses cliff/wheel-drop with no ladder; the pause alone
> strands the robot at an edge until a manual reset. The bounded escape
> response (debounce → back-off (odometry) → re-orient → resume, escalating
> to pause-and-alert at a persistent edge) is designed in
> [stair-edge-drop-off-response.md](./stair-edge-drop-off-response.md) with
> reference logic `roe/drop_off_response.py` (32 headless tests). The
> immediate-stop semantics in the two rows above are preserved verbatim; only
> the recovery path changes.
...
> Count re-measured 2026-09-08 with `/home/hermes/.local/bin/pytest test/`
> (333 = 277 prior + 32 status-emission-contract + 15 status-observability-align;
> the align suite resumes and completes the interrupted 2026-09-02 run, whose
> mid-verification constants are corrected in-tree this run; the
> emission-contract suite is new). 2026-09-08 run added
> 32 tests pinning the `oomwoo/status` emission contract: exactly-two-moments
> emission, the nine deployed payload keys, the no-keepalive liveness monitor,
> and an emitter-shape drift guard verified against the live clone (see
> [status-observability-and-emission-contract.md](./status-observability-and-emission-contract.md));
> plus 15 tests aligning external consumers' KPI vocabularies with the emitted
> lowercase state strings / UPPERCASE reason codes (PR #60 analysis, the
> 2026-09-02 run's completed work); and 9 tests grounding the wheel-drop
> NC-contact fail-safe polarity in PR #61's measured switch wiring
> (`roe/wheel_drop_failsafe.py`, also resumed from 2026-09-02).
> 2026-08-16 run added 11 tests
> guarding the in-tree launch overlay (`launch/recovery_safety.oomwoo_one.launch.py`,
> ast-verified remap + package/executable + drift/reject cases) and the
> proposed oomwoo-one bridge additions (see
> [oomwoo-one-safety-bridge-spec.md](./oomwoo-one-safety-bridge-spec.md)).
> 2026-08-18 run added 13 tests guarding the slip-diff odometry detector
> (see [slip-odometry-imu-design.md](./slip-odometry-imu-design.md)).
> 2026-08-20 run added 21 tests guarding the safety-input transport layer
> (see [safety-input-protocol-edge-semantics.md](./safety-input-protocol-edge-semantics.md)).
> 2026-08-23 run added 24 tests guarding the pause-and-alert operator
> acknowledgment (ack) path + alert re-annunciation, including a drift-guard
> that verifies the deployed upstream `recovery_node.py` still has exactly 3
> publishers (no alert topic), a one-shot status (timer never re-publishes
> status), and a True-only `_reset_cb` (see
> [pause-alert-ack-path.md](./pause-alert-ack-path.md)).
>
> 2026-09-08 re-measure note: the same pinned constants were re-verified
> against upstream/main this run — no drift; the aug23 guard constants hold
...
2. **Odometry access for STUCK_SPINNING.** The classifier needs access to
   ...
   **2026-08-18 update:** the maintained sim now publishes **two fixed
   odometry streams** — `/odom_wheel` (wheel-encoder integrated, slip shows as
   drift) and `/odom_truth` (slip-free ground-truth pose) — *always*, "for
   wheel-slip comparison … A slip detector diffs the wheel vs the ground-truth
   stream" (`makerspet/oomwoo-one` `config/gz_bridge.yaml` and
   `urdf/plugins.xacro`, both @ `jazzy`, fetched 2026-08-18). The merged node
   still subscribes no odometry, but the sim now provides the exact signal
   needed: wheel-vs-truth displacement divergence is the quantitative
   signature of spinning without progress. New reference logic
   `roe/slip_odometry.py` (13 headless tests, measured 2026-08-18) ingests
   both streams, classifies nominal / wheel-slip / immobile / external-push /
   insufficient, and optionally corroborates spin with IMU yaw-rate (`/imu`,
   also bridged). SEE
   [slip-odometry-imu-design.md](./slip-odometry-imu-design.md). Note the
   `odom_source` launch switch only changes which stream owns the CANONICAL
   `/odom`; the two fixed streams are always present, so the reference logic
   is insensitive to that switch.

3. **Wheel-drop, pickup and cliff simulation.** The current Gazebo sim does
   not model wheel-drop, pickup, **or cliff sensors** (checked against
   `alvarosamudio/oomwoo_gazebo` `urdf/plugins.xacro` — lidar + bumpers
   only). These must be manually triggered via
   `/oomwoo/safety/pickup`, `/oomwoo/safety/wheel_drop` and
   `/oomwoo/safety/cliff` until the sim model is updated. The ledge escape
   response designed in [stair-edge-drop-off-response.md](./stair-edge-drop-off-response.md)
   is testable in sim today via manual injection; automating it needs a cliff
   sensor (downward IR, per part-specs) added to the sim model.
   **2026-08-12 update:** this was re-verified against both sim repos as
   primary sources (see
   [sim-repo-recovery-verification.md](./sim-repo-recovery-verification.md))
   — neither `makerspet/oomwoo-one` nor `alvarosamudio/oomwoo_gazebo` models
   a cliff/wheel-drop sensor. Additionally, the two sims bridge **different
   bumper topic names** (`/bumper_left` vs `/bumper_left/contact`); the merged
   node matches only `oomwoo_gazebo`. Bumper-triggered recovery will not
   fire against `oomwoo-one` until the name is remapped or normalized.
   **2026-08-14 update:** re-verified against current primary sources — the
   bumper divergence is **still present** against `makerspet/oomwoo-one`
   (its gz_bridge.yaml still publishes `/bumper_left|right/contact`), and a
   NEW finding this run: that bridge's 18 entries contain **no `oomwoo/`
   topic at all**, so the node's `/oomwoo/safety/*` and `/oomwoo/recovery/*`
   inputs have no sim provider either (manual `ros2 topic pub` injection only).
   The concrete fix — a launch-time topic remap
   (`bumper_left := bumper_left/contact`, and right) verified by reference
   logic — is specified in
   [bumper-and-safety-topic-alignment.md](./bumper-and-safety-topic-alignment.md)
   and implemented headlessly in `roe/topic_alignment.py` (+12 tests).
   **2026-08-16 update:** the §5/bumper launch remap is now committed in-tree
   as an overlay launch file
   ([`launch/recovery_safety.oomwoo_one.launch.py`](./launch/recovery_safety.oomwoo_one.launch.py))
   that launches the MERGED package's node with the verified two-entry remap
   (guarded headlessly by `roe/topic_alignment.verify_launch_overlay_remap` +
   drift tests, no ROS2 import). The `oomwoo/safety/*` bridge gap is spec'd
   in [oomwoo-one-safety-bridge-spec.md](./oomwoo-one-safety-bridge-spec.md)
   with exact `gz_bridge.yaml` additions + a bridge-faithful `gz topic`
   injection recipe (landing the YAML needs a commit in `makerspet/oomwoo-one`).
   Re-verified this run that the merged node and the 18-entry oomwoo-one
   bridge are unchanged (node subscribes `oomwoo/` inputs lines 30-36;
   bridge still has no `oomwoo/` topic, `jazzy` branch).
   **2026-08-18 note:** the oomwoo-one bridge now also carries a simulated IMU
   (`/imu`, `sensor_msgs/msg/Imu`, gyro + accelerometer, GZ→ROS) and side
   short-range sensors (`/range_left`, `/range_right`, `sensor_msgs/msg/LaserScan`)
   plus a front multizone ToF (`/tof_front/points`) — all fetched 2026-08-18.
   The accelerometer enables a free-fall/impact heuristic for drop-off
   detection (candidate input for the ledge response in
   [stair-edge-drop-off-response.md](./stair-edge-drop-off-response.md)); the
   side ranges can corroborate confined-pocket wedge conditions. This remains
   a design direction, not yet reference logic (estimates; sweep in sim per
   Q5).
...
6. **Merged-node safety-input transport semantics — DESIGNED (2026-08-20).**
   Direct read of upstream/main `recovery_node.py` + `core.py` (fetched
   2026-08-20) shows the merged consumer treats `oomwoo/safety/{e_stop,
   cliff,wheel_drop,pickup}` as **level-triggered True-only** inputs: every
   safety callback is `if msg.data:` (stop + clear + trigger) with **no
   de-assert (False) branch, no debounce, no latch**, and `reset()` clears
   the controller with **no memory of the last asserted level**. Verified
   consequences: a single-sample glitch published on `oomwoo/safety/*` pauses
   the robot immediately (H1); a de-asserting sensor never self-clears the
   pause (H3); and after a `/reset` a still-asserted hazard is not noticed
   until a fresh True arrives — a transition-only producer leaves the robot
   IDLE against a live hazard (H2). Reference logic `roe/safety_input_protocol.py`
   (21 headless tests, measured 2026-08-20) models the deployed semantics
   (`MergedInputSemantics`), the producer-side contract (`ProducerAssertionPolicy`:
   debounce + transport mode + reset-aware re-assertion) and a consumer-side
   hardening latch (`ConsumerHardeningLatch`: debounce + latch + de-assert →
   PENDING_CLEAR). SEE
   [safety-input-protocol-edge-semantics.md](./safety-input-protocol-edge-semantics.md).
   The hardening latch is branch-only reference logic; wiring it into
   recovery_node.py is a future upstream node PR (not auto-PR'd).

7. **Pause-and-alert operator acknowledgment (ack path) — DESIGNED (2026-08-23).**
   The deployed node's alert surface (re-verified against upstream/main THIS
   run) is exactly: 3 publishers (`cmd_vel`, `oomwoo/status`, `oomwoo/recovery/command`)
   — **no alert topic**; status published **once** per decision with the 0.05 s
   timer never re-publishing status (no periodic annunciation); the only exit
   from PAUSED is `oomwoo/recovery/reset` (Bool, **True-only**, no else) and
   `core.reset()` clears with **no memory of the last asserted level**. Three
   hazards follow: a missed single status message leaves a stuck robot silent
   (H-A); a /reset forwarded while the hazard is still asserted resumes IDLE
   against a live hazard via the ack path (H-B, the H2 vulnerability reached
   through acknowledgment); a node restart returns READY and launders the
   alert (H-C). Reference logic `roe/pause_alert_ack.py` (24 headless tests,
   measured 2026-08-23) designs the complement: clear-before-rearm ack
   admission (`evaluate_ack_admission`), bounded re-annunciation + severity
   escalation (`AlertAnnunciator`), the full supervisor lifecycle
   (`PauseAckSupervisor`), and a drift-guard that verifies the deployed
   `recovery_node.py` still matches this model. SEE
   [pause-alert-ack-path.md](./pause-alert-ack-path.md). This closes the
   "ack path design needed before the latch can replace direct-trigger
   callbacks" item carried since 2026-08-20.

8. **Status observability & emission contract — DESIGNED (2026-09-08).**
   Re-verified against upstream/main THIS run (`recovery_node.py` blob
   `1f4d0487`, `core.py` blob `7bd5faba`, both last touched by commit
   `9a52e48` 2026-07-20): the deployed node publishes status on
   `oomwoo/status` (String) in exactly two situations — once at startup
   (`self._publish_status(self._controller.last_status)`) and once per
   controller decision (`_execute` → `_publish_status(decision.status)`) —
   and the 0.05 s timer publishes ONLY `cmd_vel` (plus the timeout path): the
   status stream has **no keepalive**, so consumer-side liveness must be
   inferred from silence. The deployed payload is
   `json.dumps(asdict(RecoveryStatus), sort_keys=True)` — exactly nine keys
   (`state, reason_code, message, recoverable, source, situation, behavior,
   step_index, ladder_length`), all present even when Optional fields are
   None. Downstream, the open upstream PR #60 baseline-observability
   collector (branch `feat/phase0-baseline-observability`, head `9439ecd39b`,
   fetched 2026-09-08) consumes exactly `payload.get("state")` /
   `payload.get("reason_code")` — a key RENAME would read as None there
   (silently "no status yet"), which is why the compat rule is additive-keys
   only. Reference logic `roe/status_emission_contract.py` (32 headless
   tests, measured 2026-09-08) ships the frame checker, a silence-based
   `StatusEmissionMonitor`, and a source-level emitter-shape drift guard run
   against the real clone in tests. SEE
   [status-observability-and-emission-contract.md](./status-observability-and-emission-contract.md).
...
- [bumper-and-safety-topic-alignment.md](./bumper-and-safety-topic-alignment.md) — merged-node ↔ sim bumper topic alignment (launch remap + safety-bridge audit, 2026-08-14; overlay launch file + bridge spec 2026-08-16)
- [oomwoo-one-safety-bridge-spec.md](./oomwoo-one-safety-bridge-spec.md) — proposed `gz_bridge.yaml` additions for `oomwoo/safety/*` (+ optional recovery control), bridge-faithful injection recipe (2026-08-16)
- [slip-odometry-imu-design.md](./slip-odometry-imu-design.md) — slip-diff odometry + IMU reference logic closing §10 Q2 on STUCK_SPINNING (2026-08-18)
