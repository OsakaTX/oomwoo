...
- **2026-09-11 status note:** upstream PR #63 is now MERGED (merge commit
  `90324ec`, merged 2026-09-11T04:20:59Z): `0x8005 SAFETY_STATE` is adopted
  contract, and `0x0004 IDENTIFY_REQUEST` (CPU→MCU) with it. If this design
  is revived, slot it at the next free id (`0x8006`) per the merged
  `conformance/protocol_v1.json` manifest — highest served id is 32773 =
  `0x8005`; `0x8006` is absent from the manifest (census taken 2026-09-11).
  OSK-008 resolution in `spec_crosscheck_20260911.md` §1; renumber, do not
  duplicate `0x8005`. (The 2026-09-09 while-open status lives in
