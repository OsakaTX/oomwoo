...
| [`docs/spec_crosscheck_20260909.md`](docs/spec_crosscheck_20260909.md) | **2026-09-09 refresh — PR #63 + PCB schematic rework.** (1) xbattlax's **PR #63 (OPEN, Sep 08)** verified against the fetched branch: new `0x0004 IDENTIFY_REQUEST` + `0x8005 SAFETY_STATE` (u32 ts, u16+u16 masks; legacy FAST_TELEMETRY byte = low 8 bits), CPU-initiated reconnect, conformance manifest + 23 golden vectors + CI; Markdown stays normative; all *proposed, unmerged*. Ledger effect: OSK-005 substance and OSK-006 doc-half would close; fork docs' `0x8005` id suggestions need renumbering once merged (annotated on this branch). (2) **First schematic+layout movement since Aug-11/Aug-23**: `f2164f7` "Schematic fixes, 1st round of review" (Sep 05, Ilia O.) touches every sheet + relayouts `Main.kicad_pcb`. **Watchdog topology redesigned**: `RTC_WATCHDOG` (PCF85063AT, CPU-fed via CM5 I2C1, `PULSE_OUT`→`PMIC_EN2`, `LATCH_OUT`→`V-MOTORS-EN`) replaced by a `WATCHDOG` sheet with one **TPS3828-33DBVR** supervisor — `MCU.WDO→WDI`, `~{JTAG_RST}`→`~MR`, `~{PULSE_OUT}` on one net with `MCU.~{STM32_RST}` + `POWER.V-MOTORS-EN` (pad-verified in the new PCB netlist: U2 pads 1/3/4/5; STM32_RST hull = U2,U13,C37,U8,R21,R58,U23,R59); `PMIC_EN2`/`LATCH_OUT` retired; CM5 I2C1 now feeds the front-ToF sensors. New **OSK-029** (firmware must define `WDO`; PR #3 silent), **OSK-030** (motor-rail gate authority redo), **OSK-031** (schematic `V-MOTORS-EN` vs PCB `POWER-EN` drift). (3) SPEC measured currents landed (drive wheel 0.14 A/1.7 A; fan 1.7/2.7 A BL24131607@15 V; main brush 0.26/3.5 A; side-brush 1.3 A *estimated*) — cross-flagged with **`oomwoo-pcb#4`** (stall-current variant reconciliation, open) fed by merged PR #61's independently characterized envelope. (4) Provenance note: pcb commits this period alternate `Ilia O. <iliao@kaia.ai>` and `Maker's Pet` (verified in log); an earlier run's `<PaulP>`-alias inference found no upstream support this run and is withdrawn. |
| [`docs/spec_crosscheck_20260911.md`](docs/spec_crosscheck_20260911.md) | **2026-09-11 refresh — PR #63 MERGED; SPEC actuator overhaul; new firmware-repo SPEC.** (1) PR #63 merged as `90324ec` (2026-09-11T04:20:59Z): `0x0004 IDENTIFY_REQUEST` + `0x8005 SAFETY_STATE` are merged contract (ids verified in the served `conformance/protocol_v1.json` = 4 / 32773; next free MCU→CPU id `0x8006`) — OSK-005 closed, OSK-006 doc-half closed, OSK-008 resolved by adoption; the 09-09 while-open status blocks in the fork docs updated same-day. (2) pcb repo: 13 commits since `8a18038`, every one `docs/SPEC.md`-only (verified per-commit) + one README badge fix; `Main.kicad_sch` sha256-identical `8a18038`↔`main` and `WATCHDOG.kicad_sch` still TPS3828 ⇒ OSK-029/030/031 evidence stands; SPEC tip `e4632afa`. (3) SPEC rebuilt ~201→353 lines: new "Reverse engineering data" table — drive wheel `RS-360-SH-15250`, 1.7 A@14.4 V / 2 A@16.8 V, 19 Ohm retired per maintainer in `oomwoo-pcb#4`; nine fan rows, ALL now `PWM (low off)` + FG open-collector (five with measured currents; production fan still unnamed; BL24131607 PH2.0-5p vs board 4p wafer = OSK-028 open); FlexiArm side-brush pins `3 IR output? 4 IR GND? 5 IR VDD?` maintainer-unknown ⇒ OSK-002 pressure; mop drums 5.3/5 A; five 5 V water-pump vendor rows (section re-rated 6 V→5 V); new "How to drive carpet sensor" (STM32G473 op-amp/DAC/ADC chain). (4) New primary source `makerspet/oomwoo-io-firmware` `docs/SPEC.md` (`b9d5b4ca`): mirrors the carpet-sensor chain plus maintainer policy — ≈13 A ≈2.5 C budget ⇒ throttle fan on brush/drive current, 0.8× stall is a backstop not jam detection, RTC backup-loss ⇒ refuse schedules until time re-established (inputs to `POWER_TELEMETRY`/`SAFETY_EVENT` parametrization; OSK-009/010 evidence). (5) SPEC gains `Pi CM5 worst case ~15.6W` and the explicit "MCU reset drops everything to a safe state … - watchdog" safety line. (6) Watch: firmware #1/#2/#3 open, PR #3 body updated 09-11 (Nucleo-G474RE HIL, 150 ms gate still pending #1, production pin map + IWDG deferred ⇒ OSK-029 re-confirmed); `oomwoo-pcb#4` open, thread active; main repo #60 only open PR, #64 (serengon, dock auto-empty fan) merged 09-11 — adjacent module, not cross-checked here. |
| [`docs/spec_crosscheck_20260913.md`](docs/spec_crosscheck_20260913.md) | **2026-09-13 refresh — PCB "second review round".** 15 pcb commits `e4632afa`→`4f104bea` rewrite the sheet tree: root now **21 wired sheets** with **CLIFF** (`ANTI-FALL-*` ×4, `CLIFF_LED_EN`, `BUMPER_LED_EN`) and **Carpet-sensor** (`CARPET-SENSOR-HI/LO/ECHO`; RS8422XK ×2 + BJT drive chain) wired in for the first time. **Watchdog supervisor swapped again — TPS3828 gone, `U3 STWD100NYWY3F`** (`WDI`/`~{PULSE_OUT}` kept, `~{MR}` dropped, `~{EN}` new) ⇒ OSK-029 re-based, with **PD8=`WDO` pinned in the maintainer IO xlsx**, whose new net-assignment columns also land `PD9=LIDAR_EN` (new MCU-side LiDAR enable ⇒ OSK-023 must sequence it), `PA15=STM-PWR-CTRL`, `PB13=RK-RESET`, carpet/encoder/doc-IR pins (Appendix A = verbatim parsed diff). SPEC delta is fan "fits \<machine\> (kPa)" data + the maintainer's `* Appear to be interchangeable` note (**OSK-028** input) + `JST PAD`→`PHDR-16VS` cliff-connector rename — **no signal or pinout change**; the G070-named MCU sheet now verifiably holds a G473 symbol (**OSK-018**); `LIDAR_EN` vs `LiDAR-EN` spelling drift flagged (OSK-031 pattern). Standing docs' 09-13 status blocks added. |
| [`docs/spec_crosscheck_20260920.md`](docs/spec_crosscheck_20260920.md) | **2026-09-20 refresh — wire v2 + typed codec MERGED upstream.** (1) Firmware: PR #2 framing core merged 09-15 (`2d51eff5`), PR #4 typed codec merged 09-19 (`213c16f8`) — the canonical machine-readable registry `tests/conformance/protocol_v1.json` now exists: 16 messages with per-message `struct_format`, `crc CRC-16/CCITT-FALSE`, ids 0x0001…0x8005, next free MCU→CPU id `0x8006` (verified by parsing the fetched JSON; the 09-11 projection confirmed) ⇒ id/payload tables across the module's docs are registry-derivable. (2) PR #3 watchdog core re-based on #2 (head `69bcf370`, open, clean; its range now also carries the `oomwoo_protocol.c` framing implementation) and NEW PR #5 MCU ingress gate (open, clean) — neither merged at run time; #5 explicitly does not refresh the heartbeat watchdog (gate/watchdog separation, matching the no-CPU-pin heartbeat record). (3) pcb 6 commits `4f104bea`→`22e3a597`: master-fuse rev lands dedicated **`STM32G473.kicad_sch`** (root references it; legacy `STM32G070RBT6.kicad_sch` now an unreferenced duplicate, 0 G070 / 13 G473 strings) ⇒ **OSK-018 closed upstream-side**; `CM5-GPIO` rewrite censused at hier-label level — CPU-side surface exactly the standing set (`PMIC_EN`, `RUN_PG`, `UART2_RX/TX`, `I2S_*`, `SDA0/1`; no new CPU heartbeat pin); another `Main.kicad_pcb` relayout (pad-truth re-derivation still owed). (4) SPEC 365→405 L: new `IR receiver ICs` section + motor/sensor row rework (`22e3a597` +90/−69, parsed at section level only); still zero `uart`/`baud` strings — wire-level truth lives in the fw repo; GPIO section still links pre-rename `oomwoo-io-board` (OSK-022 pattern, +1 occurrence). Standing docs' 09-20 status blocks added. |
| [`docs/spec_crosscheck_20260922.md`](docs/spec_crosscheck_20260922.md) | **2026-09-22 refresh — the CPU/MCU interface grows a POWER HANDSHAKE.** pcb `a7ac0fd7` "STM32 power rail switch" (09-21, 51 files, parent `22e3a597`) adds hier labels `3.3V_EN`(PB3)/`5V_EN`(PB4)/`USB_PWR_EN`(PC10) as MCU outputs + `PI_DONE`(PB5↔CM5-GPIO output) as MCU input, joined at root and landed as inputs on the rebuilt `BMS-SYSTEM-POWER` (HT7533-1/MT3608/AP64501 era; parts presence-swept, datasheets NOT fetched) — the 09-20 "24-label standing set" CPU census is one commit stale; new ledger item **OSK-033**: PB13 `RK-RESET` vs alt-fn `FDCAN2_TX` dual-role (open, High). fw **PR #5 ingress gate MERGED `d103a5d4`** (09-21) ⇒ PR #3 watchdog core's dependencies ALL merged; OSK-029 ratification owed at merge; STWD100NYWY3F datasheet unfetched 4th run. WATCHDOG/LiDAR/MAIN-FAN sheets label-unchanged; `Main.kicad_pcb` relayout #3 ⇒ OSK030/031 pad debt now spans three layouts. ids/registry no drift; main repo quiet since 09-19. Full ledger + §9 not-verifier list in the doc. |
| [`docs/spec_crosscheck_20260924.md`](docs/spec_crosscheck_20260924.md) | **2026-09-24 refresh — `PI_SHUTDOWN` completes the power handshake x2.** pcb `78bd659c` "LDR" (09-22, 11 files): new hier label `PI_SHUTDOWN` verified at all four levels (MCU out / CM5 in / root one-wire / xlsx **PB6**) ⇒ the up-leg (3.3V_EN/5V_EN/USB_PWR_EN MCU-out, `PI_DONE` PC-ack) gains its teardown leg with **no semantics anywhere** — new **OSK-034** (High; maintainer, pre-HIL). xlsx delta is exactly 3 pin rows (`PB6`=PI_SHUTDOWN new; `PC13`=LED-HOME vs `PB12` kept-as-alias ⇒ **OSK-035** dual-home; PB12 primary→`LDR`); GL5537-1 LDR+10k (R75/R103) = new MCU-owned analog input, message-less; `MCU_IO1` header removed (contract-silent). **OSK-029 numeric side CLOSED**: STWD100NYWY3F datasheet fetched (DocID14134 Rev 11) — xY family 1.12/1.6/2.24 s, tPW 140/210/280 ms, WDI ≥1 μs both-edges ⇒ kick duty ≤1.6 s ⋙ fw's 150 ms debate; hw supervisor = last-resort, now manufacturer-backed. Registry re-parse at fw `d103a5d4`: 16 ids, next `0x8006`, no drift. |
| [`docs/spec_crosscheck_20260926.md`](docs/spec_crosscheck_20260926.md) | **2026-09-26 refresh — watchdog second stage + debug/reset ownership shift + I²S audio silicon.** pcb 3 commits 09-26: SPEC audio notes (`7326520f` NS4168 = MAX98357A clone; `5011752e` RP2040-USB mic array + **"UART0 GPIO14/15 console; if the STM32 link uses UART0, move it"** — wired reality is already USART1↔CM5 UART2, recorded) and `c9b9c868` (71 files): WATCHDOG sheet gains **U4 `TP74LVC1G332S6` 3-input OR** with new `DIS1/2/3` = MCU `JTAG_PRESENCE` / SWDIO-pair / `BOOT`←CM5 `BOOT0`; **CM5 `~{STM_RST}` output joins `~{PULSE_OUT}`/`~{STM32_RST}`/`V-MOTORS-EN`** (OSK-024 pair gains a CPU driver) ⇒ **OSK-037** (fw must define CPU-reset/debug vs WDI). MCU↔charger `I2C3`+`~{CHG_INT}` boundaries removed both sides ⇒ **OSK-036** (replacement charger-status sense point undocumented). SPEAKER sheet (2× NS4168) wired to CM5 `I2S_*`+`GPIO011`→`SP_SHUTDOWN` — CPU-owned, no contract message. Maintainer xlsx NOT updated in-commit (new pins xlsx-unmapped); `Main.kicad_pcb` 5th relayout (+893/−247; OSK-030/031 debt now five layouts). fw unchanged (`d103a5d4`; PR #6 open, docs-only); main repo PR #68 = adjacent-module CI, noted not reviewed. |
