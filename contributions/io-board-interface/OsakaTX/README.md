...
| [`docs/bumper_cliff_signal_remap_20260916.md`](docs/bumper_cliff_signal_remap_20260916.md) | **2026-09-16 POP — bumper/cliff connector reverse-engineered (`8b40d5fb`).** CLIFF sheet loses both phototransistor symbols and its four `ANTI-FALL-*-ADC` hierarchical labels, gaining photodiode-quadrant inputs `RF/RR/LF/LR-Q+` **plus new bumper channels `BUMP-L-Q+`/`BUMP-R-Q+`** (all six verified onto MCU ADC pins via the maintainer IO xlsx, verbatim cell diff: PA0/PA1/PA2/PA3 cliff, PA4/PA5 bumper; side effect: `MAIN-BRUSH-CURRENT-ADC` PA4→PA6, `WATER-PUMP-SENSE-ADC` PA5→PA7, `LED-PWR` PA6→PF2). Interprets the bank as a shared-photodiode cliff+bumper assembly per side selected by `CLIFF_LED_EN`/`BUMPER_LED_EN`, lists contract consequences (`SAFETY_EVENT` 1–6 source semantics; `FAST_TELEMETRY` analog scaling; LED-state context gap) — **OSK-033**, open for maintainer/PCB-designer. |
| [`docs/spec_crosscheck_20260916.md`](docs/spec_crosscheck_20260916.md) | **2026-09-16 crosscheck.** OSK-033 intake + ledger deltas: pcb `c6d509c4`/`05b2896c`/`5bfce6e9` SPEC-only edits (new `### IR receiver ICs` section, 38 kHz NEC requirement, 8 alternate SKUs with prices/stock as written by the maintainer — quoted, not re-verified); firmware `oomwoo-io-firmware#1` now records wire-stays-v1 + `SAFETY_STATE`-authoritative as decided, Open items = 150 ms heartbeat confirm + disarmed-telemetry policy; new fw PR #4 typed codec (open), #2 framing merged 09-15; served `protocol_v1.json` sha-identical to in-tree, top id still `0x8005`, next free `0x8006`; OSK-024 re-based onto the new bumper photodiode reality; OSK-022/028/031 status refreshed with this-run evidence. |
