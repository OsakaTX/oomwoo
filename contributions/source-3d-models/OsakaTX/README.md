...
| **Robot Suction Fan — alternate BOM options** (5.1-6 / 10 / 36 / 2-2.5 kPa rows) | NEW 2026-09-15 — envelope-class draft, 3 presets: legacy_od70 (screenshot-class 70/40, image-measured), mid_od58 / large_od60 (Nidec deck-published Φ58×L63.3 / Φ60×L73.3 exact); twin-inlet + side-exit + ear topology per reference photos | `robot-suction-fan-alternates/alternate-housing-envelope.scad` + `robot-suction-fan-alternates/jig26-fan-class-gauge.scad` (Jig 26, 3 plates + ring-half split) | Covers BOM kPa-option rows L25-L28 (Nidec 22N704W150/20N704S980/20N704R980L, BL24131616/22N704V160, Saros 20 unit, 20N704P200-class) — flagged `❌ Missing` since the first cross-ref. **No primary datasheet exists for any exact suffix** (2-round catalogue search 2026-09-15: Nidec sitemap + web; only family pages publish data — DC vacuum motor "φ65~150 mm" class blades; deck covers BL-V series, not 20N/22N). Mid/large preset dims render exact to the deck values (STL bbox 63.30/73.30 axial). Jig 26 identifies the purchased unit's class with no caliper; MEASURE-ME §25 rows 2+3 then lock the preset | ⚠️ Envelope-class — deck anchors [B] / legacy [I] / internals (E); exact-suffix model waits on the purchased unit |
...
| Suction fan (other kPa) | `robot-suction-fan-alternates/alternate-housing-envelope.scad` — envelope-class, 3 presets (`legacy_od70` / `mid_od58` / `large_od60`); NEW 2026-09-15 | NO primary datasheet for the exact suffixes (2-round search 2026-09-15, see model header); anchors: Nidec blower-deck p.5/p.9/p.14 published Φ/L values [B]; legacy preset image-measured; Wikimedia CC topology photos | ⚠️ Envelope-class (was `❌ Missing`) — preset locked by Jig 26 + MEASURE-ME §25 rows 2/3 |
